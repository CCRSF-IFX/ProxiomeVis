...
test_that("each readout tab has only the controls it needs", {
  ...
  expect_true(grepl('id="colocalization-colocalization_pixelator_filter_enabled"', html, fixed = TRUE))
  expect_true(grepl('id="colocalization-colocalization_background_threshold_pct"', html, fixed = TRUE))
  ...
  expect_true(grepl("closer than expected by chance", html, fixed = TRUE))
  expect_true(grepl("approximately random spatial organization", html, fixed = TRUE))
  expect_true(grepl("spatial segregation", html, fixed = TRUE))
...
test_that("resizable plots keep size controls with the plot pane controls", {
  ...
  expect_false(grepl('sliderInput(ns("abundance_point_size")', abundance_sidebar_source, fixed = TRUE))
  expect_false(grepl('checkboxInput(ns("abundance_distribution_show_jitter")', abundance_sidebar_source, fixed = TRUE))
})
...
test_that("navbar exposes server-side RDS path loading on HPC and desktop only", {
  ...
  expect_true(grepl('id="data_source-configure_analysis_grouping"', html, fixed = TRUE))
  expect_true(grepl('id="data_source-analysis_grouping_summary"', html, fixed = TRUE))
  expect_true(grepl("Analysis grouping", html, fixed = TRUE))
